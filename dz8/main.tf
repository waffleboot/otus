...
resource yandex_lb_network_load_balancer postgresql_load_balancer {
  type = "internal"
  listener {
    name = "haproxy"
    port = var.postgresql_load_balancer.port
    internal_address_spec {
      subnet_id = yandex_vpc_subnet.subnet.id
      address = var.postgresql_load_balancer.addr
    }
  }
  attached_target_group {
    target_group_id = yandex_lb_target_group.postgresql_target_group.id
    healthcheck {
      name = "tcp"
      tcp_options {
        port = var.postgresql_load_balancer.port
...
resource yandex_lb_target_group postgresql_target_group {
  target {
    subnet_id = yandex_vpc_subnet.subnet.id
...
resource local_file inventory-ini {
  content = templatefile("inventory.tftpl",{
    ...
    nginx_load_balancer_addr = var.nginx_load_balancer.addr
    nginx_load_balancer_port = var.nginx_load_balancer.port
    postgresql_load_balancer_addr = var.postgresql_load_balancer.addr
    postgresql_load_balancer_port = var.postgresql_load_balancer.port
  })
